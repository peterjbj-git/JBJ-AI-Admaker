...
5. **컷별 비디오 생성** (`generate_video`) — 스토리보드의 컷 단위로, 순서대로 생성한다.
   비율은 brief의 비율과 일치시킨다.
   ...
   - 크레딧 계획은 **컷 수 기준**으로 세운다 (컷 단가 = 클립 단가).
...
- **컷 다양성**: 같은 씬 안의 컷들은 스타일 문구는 공유하되 **앵글·거리·움직임을 다르게** 한다
  (와이드→클로즈업, 정면→측면, 고정→푸시인 등). 같은 구도의 컷이 연달아 나오면 컷을 나눈 의미가 없다.
- **오디오 지시 필수**: 모든 씬 프롬프트 끝에 `"ambient sound effects only, no background music"` 을
  붙인다 (audio-guide 0절 계약 — 클립 내장 음악은 씬 전환마다 끊기는 결함을 만든다).
- **카메라 무브 명시·기록**: 각 컷의 카메라 무브(dolly in / pan left / static 등)를 프롬프트에 쓰고,
  scenes.json의 `camera` 필드에 같은 값을 기록한다 — 자막 진입 모션이 카메라와 정합된다(L4).
